...
@router.callback_query(F.data == "get_tasks_from_day", StateFilter(default_state))
...
@router.callback_query(F.data[:13] == "calendar_get_", StateFilter(default_state))
...
async def get_tasks(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=show_tasks_keyboard(tasks[:MAX_OBJECTS_ON_PAGE])
    )
...
async def get_tasks_from_category(callback: CallbackQuery, state: FSMContext):
    ...
    pages = len(tasks) // MAX_OBJECTS_ON_PAGE + int(len(tasks) % MAX_OBJECTS_ON_PAGE != 0)
    await state.update_data(
        tasks_list=tasks, tasks_pages=pages, cur_tasks_page=1,
        lexicon_data="tasks_from_category"
    )

    await callback.message.edit_text(
        text=TASKS_LEXICON["tasks_from_category"].format(name=category["name"], page=1, pages=pages),
        parse_mode="Markdown",
        reply_markup=show_tasks_keyboard(tasks[:MAX_OBJECTS_ON_PAGE])
    )


@router.callback_query(lambda c: c.data in ("next_task_page", "prev_task_page"))
async def change_page(callback: CallbackQuery, state: FSMContext):
    user_data = await state.get_data()
    tasks = user_data["tasks_list"]
    pages = user_data["tasks_pages"]
    cur_page = user_data["cur_tasks_page"]
    lexicon_data = user_data["lexicon_data"]

    change_keyboard_flag = True
    if callback.data == "next_task_page":
        if cur_page < pages:
            cur_page += 1
            await state.update_data(cur_tasks_page=cur_page)
        else:
    ...
    if change_keyboard_flag:
        await callback.message.edit_text(
            text=TASKS_LEXICON_COMMANDS[lexicon_data].format(page=cur_page, pages=pages),
            reply_markup=show_tasks_keyboard(
                tasks[(cur_page - 1) * MAX_OBJECTS_ON_PAGE: cur_page * MAX_OBJECTS_ON_PAGE]
            )
        )
...
@router.callback_query(F.data[:14] == "calendar_page_")
...
@router.callback_query(F.data[:21] == "calendar_select_year_")
...
@router.message(StateFilter(TaskState.get_year))
...
async def no_edit_description(callback: CallbackQuery, state: FSMContext):
    ...
    data = set_edited_task_data(task, "description", "")
    task = await task_service.edit_task(task_id, data, user_data["access_token"])
    task_category = await category_service.get_category(task["category_id"], user_data["access_token"])
